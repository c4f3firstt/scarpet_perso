...
__on_player_right_clicks_block(player, item_tuple, hand, block, face, hitvec) -> (
	if(global_active && hand == 'mainhand' && item_tuple && item_tuple:0 =='iron_pickaxe',
...
__on_player_clicks_block(player, block, face) -> (
	if (global_radius && global_active,
		...
		if (!item_mainhand || !(item_mainhand:0 == 'iron_pickaxe'), return());
...
__on_player_breaks_block(player, block) -> (
	...
	if (global_active && global_radius && block_range,
		item_mainhand = player ~ 'holds';
		if(item_mainhand && item_mainhand:0 == 'iron_pickaxe',
